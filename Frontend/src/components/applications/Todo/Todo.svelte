<script lang="ts">
    ...
    async function fetchTodos(forceReload = false) {
        try {
            const response = await sendRequest('reportboard.agency/daily', 443, 'GET', '/todos/', null);
            if (response.todos) {
    ...
    async function addTodo() {
        ...
        if (newText) {
            await sendRequest('reportboard.agency/daily', 443, 'POST', '/todo/', { content: newText });
            fetchTodos();
        }
    }
    ...
    try {
        ...
        await sendRequest('reportboard.agency/daily', 443, 'PUT', `/todo/${id}`, { completed: !completed });
        fetchTodos();
    } catch (error) {
    ...
    async function editTodo(todoId: number) {
        ...
        if (newText) {
            try {
                await sendRequest('reportboard.agency/daily', 443, 'PUT', `/todo/edit/${todoId}`, { content: newText });
                fetchTodos();
            } catch (error) {
    ...
    async function deleteTodo(todoId: number) {
        try {
            await sendRequest('reportboard.agency/daily', 443, 'DELETE', `/todo/${todoId}`, {});
